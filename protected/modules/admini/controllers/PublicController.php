        ...
        if (XUtils::method() == 'POST') {
            ...
            if ($model->validate()) {
                ...
                if ($data === null) {
                    ...
                    parent::_adminiLogger(array ('catalog' => 'login' , 'intro' => '登录失败，用户不存在:' . CHtml::encode($model->username) , 'user_id' => 0 ));
                } elseif (! $model->validatePassword($data->password)) {
                    $model->addError('password', '密码不正确');
                    parent::_adminiLogger(array ('catalog' => 'login' , 'intro' => '登录失败，密码不正确:' . CHtml::encode($model->username). '，使用密码：'.CHtml::encode($model->password) , 'user_id' => 0 ));
