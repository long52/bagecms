...
<div class="global clear">
      ...
      <div class="border">
        <div id="portal_block_237_content" class="dxb_bc">
          <div class="module cl xld slideshow">
            <?php foreach((array)Bagecms::getList('Calendar','hot_goods',array('where'=>"status_is='Y' AND catalog_id=5",'order'=>'view_count DESC', 'limit'=>2)) as $goodsKey=>$goodsRow):?>
			<dl class="clear">
              <dd class="m"><a href="<?php echo $this->createUrl('cal/show',array('id'=>$goodsRow['id']))?>" target="_blank"><img src="<?php echo $this->_baseUrl?>/<?php echo $goodsRow['attach_thumb']?>" width="150" height="115"></a></dd>
             
              <dt><a href="<?php echo $this->createUrl('cal/show',array('id'=>$goodsRow['id']))?>" title="<?php echo $goodsRow['title']?>" target="_blank"><?php echo $goodsRow['title']?></a></dt>
            </dl>
           <?php endforeach?>
          </div>
        </div>
  ...
  <div class="mainBox floatL">
    <div class="loc clear">
      <div class="floatL position"> <span>您的位置：</span> <a href="<?php echo Yii::app()->homeUrl?>">首页</a> <em></em><span>产品中心</span><em></em> <a href="<?php echo $this->createUrl('calendar/index',array('catalog'=>$catalogArr['id']))?>"><?php echo $catalogArr['catalog_name']?></a> <em></em> 查看内容 </div>
    </div>
    <div class="postWrap">
      <div class="h head">
      ...
      <!--上下篇-->
    <?php $before = Bagecms::getList('Calendar', 'pageBefore', array('where'=>"id<{$bagecmsShow['id']} AND catalog_id={$bagecmsShow['catalog_id']} AND status_is='Y'",'order'=>'id DESC', 'limit'=>1))?>
    <?php $behind = Bagecms::getList('Calendar', 'pageBehind', array('where'=>"id>{$bagecmsShow['id']} AND catalog_id={$bagecmsShow['catalog_id']} AND status_is='Y'",'order'=>'id ASC', 'limit'=>1))?>
    <div class="preNext clear" > <em class="floatL">上一篇：
      <?php if($before):?>
      <a href="<?php if($before[0]['redirect_url']):?><?php echo XUtils::convertHttp($before[0]['redirect_url'])?><?php else:?><?php echo $this->createUrl('cal/show',array('id'=>$before[0]['id']))?><?php endif?>"><?php echo $before[0]['title']?></a>
      <?php else:?>
      没有了
      <?php endif?>
      </em><em class="floatR">下一篇：
      <?php if($behind):?>
      <a href="<?php if($behind[0]['redirect_url']):?><?php echo XUtils::convertHttp($behind[0]['redirect_url'])?><?php else:?><?php echo $this->createUrl('cal/show',array('id'=>$behind[0]['id']))?><?php endif?>"><?php echo $behind[0]['title']?></a>
      <?php else:?>
      没有了
      <?php endif?>
      </em></div>
