...
<!--/左侧导航-->
  ...
  <div class="mainBox">
    <div class="loc clear">
      <div class="position"> <span>您的位置：</span> <a href="<?php echo Yii::app()->homeUrl?>">首页</a> <em></em> <span>日程</span><em></em><a href="<?php echo $this->createUrl('cal/index',array('catalog'=>$catalogArr['id']))?>"><?php echo $catalogArr['catalog_name']?></a>  </div>
    </div>
    <div class="postWrap">
      <div class="h head">
    ...
    <div id="relatedPost" class="bm">
      ...
      <div class="bmc">
		...
		<?php foreach((array)Bagecms::getList('Calendar','index_news',array('where'=>"status_is='Y' AND catalog_id={$bagecmsShow['catalog_id']} AND id!={$bagecmsShow['id']}",'order'=>'id DESC', 'limit'=>8)) as $newsKey=>$newsRow):?>
          <li>• <a href="<?php if($newsRow['redirect_url']):?><?php echo XUtils::convertHttp($newsRow['redirect_url'])?><?php else:?><?php echo $this->createUrl('cal/show',array('id'=>$newsRow['id']))?><?php endif?>" target="_blank"><?php echo $newsRow['title']?></a></li>
		<?php endforeach?> 
        </ul>
      </div>
    </div>
