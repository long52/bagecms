           ...
           <?php foreach((array)$bagecmsDataList as $bagecmsKey=>$bagecmsRow):?>
            <li class="clear ">
              <h2>
                ...
                <a href="<?php if($bagecmsRow->redirect_url):?><?php echo XUtils::convertHttp($bagecmsRow->redirect_url)?><?php else:?><?php echo $this->createUrl('cal/show',array('id'=>$bagecmsRow->id))?><?php endif?>" target="_blank" class="title" <?php if($bagecmsRow->title_style):?>style="<?php echo $bagecmsRow->title_style?>"<?php endif?>><?php echo $bagecmsRow->title?></a> </h2>
            </li>
            <?php endforeach?>
